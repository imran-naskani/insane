...
def evaluate_option_exit(prices: deque):
    if len(prices) < 4:
        return False

    # 1️⃣ No higher high in last 3 mins
    recent_max = max(list(prices)[:-1])
    no_new_high = prices[-1] < recent_max

    # 2️⃣ Momentum flattening
    slope = np.mean(np.diff(list(prices)[-4:]))

    # Exit if BOTH true
    return no_new_high and slope <= 0
...
ib = IB()
ib.connect('127.0.0.1', 4002, clientId=502, timeout=5)
        ...
        for sym, cfg in INSTRUMENTS.items():
            ...
            if signal:
                ...
                if opt_price:
                    ...
                    # cds = ib.reqContractDetails(cfg["contract"])
                    # con = cds[0].contract

                    # opt_contract = FuturesOption(
                    #     symbol=cfg["option_symbol"],
                    #     lastTradeDateOrContractMonth=opt_symbol.split()[0][-3:],  # acceptable
                    #     strike=float(opt_symbol.split()[-1]),
                    #     right="C" if direction == "CALL" else "P",
                    #     exchange=con.exchange,
                    #     currency=con.currency
                    # )
